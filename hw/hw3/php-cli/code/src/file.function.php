...
function birthdayFunction(array $config): string
{
    ...
    if (file_exists($address) && is_readable($address)) {
        ...
        if ($contents !== '') {
            return "Сегодня поздравить с Днем рождения: " . PHP_EOL . $contents;
        } else {
...
function deleteFunction(array $config): string
{
    ...
    $searchName = trim(ucfirst(readline('Введите имя для поиска: ')));
    $searchName .= " " . trim(ucfirst(readline('Введите фамилию для поиска: ')));

    if (!validateName($searchName)) {
        return handleError("Неверно введено имя или фамилия");
    }
    ...
    if (file_exists($address) && is_readable($address) && is_writable($address)) {
        ...
        while (!feof($file)) {
            $string = fgets($file);
            if (explode(', ', $string)[0] === $searchName) continue;
            $content .= $string;
        }
        fclose($file);
